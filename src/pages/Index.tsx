...
/* ─── Data ─── */
const features = [
  { icon: Bot, title: "Custom AI Agents", desc: "Build voice assistants with custom personalities, prompts, and behaviors tailored to your use case.", color: "from-violet-500 to-purple-600" },
  ...
  { icon: Shield, title: "Enterprise Ready", desc: "SOC2 compliant, end-to-end encryption, and role-based access for production deployments.", color: "from-indigo-500 to-blue-600" },
];
...
const GridBackground = () => (
  <div className="pointer-events-none absolute inset-0 overflow-hidden">
    ...
    <motion.div
      ...
      className="absolute top-0 right-1/3 w-px h-full bg-gradient-to-b from-transparent via-indigo-500/15 to-transparent"
    />
  </div>
);
    ...
    {Array.from({ length: count }).map((_, i) => (
      <motion.div
        ...
        style={{
          ...
          top: `${Math.random() * 100}%`,
          background: Math.random() > 0.5 ? 'rgb(139,92,246)' : 'rgb(99,102,241)',
        }}
      />
    ))}
  </div>
...
const HeroOrb = () => (
  <div className="relative flex items-center justify-center">
    ...
    <motion.div
      ...
      className="absolute h-72 w-72 rounded-full border border-violet-500/10"
    >
      <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 h-2 w-2 rounded-full bg-violet-400 shadow-[0_0_12px_rgba(139,92,246,0.8)]" />
    </motion.div>

    <motion.div
      animate={{ rotate: -360 }}
      transition={{ duration: 30, repeat: Infinity, ease: "linear" }}
      className="absolute h-56 w-56 rounded-full border border-indigo-500/10"
    >
      <div className="absolute bottom-0 right-0 h-1.5 w-1.5 rounded-full bg-indigo-400 shadow-[0_0_10px_rgba(99,102,241,0.8)]" />
    </motion.div>

    <motion.div
      animate={{ rotate: 360 }}
      transition={{ duration: 15, repeat: Infinity, ease: "linear" }}
      className="absolute h-40 w-40 rounded-full border border-violet-400/15"
    >
      <div className="absolute top-1/2 right-0 translate-x-1/2 -translate-y-1/2 h-1 w-1 rounded-full bg-violet-300 shadow-[0_0_8px_rgba(139,92,246,0.6)]" />
    </motion.div>
    ...
    <motion.div
      ...
      transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }}
      className="absolute h-52 w-52 rounded-full bg-violet-600/20 blur-[60px]"
    />
    ...
    <motion.div
      ...
      className="relative h-28 w-28 rounded-full bg-gradient-to-br from-violet-500 via-indigo-500 to-purple-600 shadow-[0_0_60px_rgba(124,58,237,0.4)]"
    >
...
const Index = () => {
  ...
  const spotlightBg = useTransform(
    [smoothX, smoothY],
    ([x, y]: number[]) =>
      `radial-gradient(600px 350px at ${(x * 100).toFixed(1)}% ${(y * 100).toFixed(1)}%, rgba(124,58,237,0.07), transparent 70%)`
  );
      ...
      <header className="sticky top-0 z-50 border-b border-white/5 bg-zinc-950/80 backdrop-blur-xl">
        <div className="container flex h-14 items-center justify-between">
          <Link to="/" className="flex items-center gap-2.5 font-display text-base font-bold tracking-tight">
            <div className="relative h-8 w-8 overflow-hidden rounded-lg border border-white/10 bg-white/5 grid place-items-center">
              <AudioWaveform className="h-4 w-4 text-violet-400" />
            </div>
            <span className="bg-clip-text text-transparent bg-gradient-to-r from-white to-white/70">VOXAI</span>
          </Link>
          ...
          <div className="flex items-center gap-2">
            ...
            <Button asChild size="sm" className="bg-violet-600 hover:bg-violet-500 text-white shadow-lg shadow-violet-600/20">
      ...
      <main>
        ...
        <section ref={heroRef} className="relative overflow-hidden min-h-[90vh] flex items-center">
          ...
          <motion.div
            ...
            className="pointer-events-none absolute top-20 -left-32 w-96 h-96 bg-violet-600/10 rounded-full blur-[120px]"
          />
          <motion.div
            animate={{ x: [0, -40, 30, 0], y: [0, 30, -30, 0], scale: [1, 0.9, 1.1, 1] }}
            transition={{ duration: 15, repeat: Infinity, ease: "easeInOut" }}
            className="pointer-events-none absolute bottom-20 -right-32 w-80 h-80 bg-indigo-600/10 rounded-full blur-[120px]"
          />
          ...
          <div className="container relative flex flex-col lg:flex-row items-center gap-16 px-4 py-20 lg:py-0">
            ...
            <div className="flex-1 max-w-2xl">
              <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}
                className="mb-6 inline-flex items-center gap-2 rounded-full border border-violet-500/20 bg-violet-500/5 px-4 py-1.5 text-xs text-violet-400">
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.4 }}
                ...
                <Button asChild size="lg" className="bg-violet-600 hover:bg-violet-500 text-white shadow-lg shadow-violet-600/25 h-12 px-6">
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }}
                ...
                {[
                  ...
                  <div key={s.label}>
                    <p className="font-display text-xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-violet-400 to-indigo-400">{s.val}</p>
          ...
          <div className="container relative px-4">
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-80px" }} variants={fadeUp} custom={0} className="mb-16 text-center">
              <motion.div animate={{ opacity: [0.5, 1, 0.5] }} transition={{ duration: 3, repeat: Infinity }}
                className="mb-4 inline-flex items-center gap-2 rounded-full border border-violet-500/20 bg-violet-500/5 px-3 py-1 text-xs text-violet-400">
              ...
              {features.map((f, i) => {
                ...
                return (
                  <motion.div key={f.title} variants={fadeUp} custom={i}
                    ...
                    className="group relative rounded-2xl border border-white/5 bg-zinc-900/50 p-6 transition-all duration-300 hover:border-violet-500/20 hover:bg-zinc-900/80 cursor-default overflow-hidden">
                    {/* Hover glow */}
        ...
        <section id="how-it-works" className="relative py-24 lg:py-32 overflow-hidden">
          ...
          <motion.div
            ...
            className="absolute top-1/2 left-0 w-full h-px bg-gradient-to-r from-transparent via-violet-500/30 to-transparent"
          />
              ...
              {steps.map((s, i) => {
                ...
                return (
                  <motion.div key={s.num}
                    ...
                    whileHover={{ y: -6 }}
                    className="group relative rounded-2xl border border-white/5 bg-zinc-900/50 p-6 transition-all duration-300 hover:border-violet-500/20">

                    <motion.span
                      animate={{ opacity: [0.1, 0.2, 0.1] }}
                      transition={{ duration: 3, repeat: Infinity, delay: i * 0.5 }}
                      className="font-display text-5xl font-bold text-violet-500/10 absolute top-4 right-4"
                    >{s.num}</motion.span>

                    <div className="relative mb-4 inline-flex h-10 w-10 items-center justify-center rounded-xl bg-violet-500/10 text-violet-400 group-hover:bg-violet-500/20 transition-colors">
                      <Icon className="h-4 w-4" />
                    </div>
        ...
        <section className="relative py-24 lg:py-36 overflow-hidden">
          ...
          <motion.div
            ...
            transition={{ duration: 30, repeat: Infinity, ease: "linear" }}
            className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] rounded-full border border-violet-500/5"
          />
          <motion.div
            animate={{ scale: [1.2, 1, 1.2], rotate: [360, 180, 0] }}
            transition={{ duration: 25, repeat: Infinity, ease: "linear" }}
            className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[350px] h-[350px] rounded-full border border-indigo-500/5"
          />

          <motion.div
            animate={{ scale: [1, 1.3, 1], opacity: [0.1, 0.25, 0.1] }}
            transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }}
            className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-72 h-72 bg-violet-600/15 rounded-full blur-[100px]"
          />
          ...
          <div className="container relative px-4">
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-80px" }} variants={fadeUp} custom={0}
              ...
              <h2 className="font-display text-3xl font-bold tracking-tight sm:text-4xl lg:text-5xl">
                The Future of <span className="text-transparent bg-clip-text bg-gradient-to-r from-violet-400 to-cyan-400">Voice Interaction</span>
              </h2>
              ...
              <div className="mt-10 flex flex-wrap items-center justify-center gap-4">
                <Button asChild size="lg" className="bg-violet-600 hover:bg-violet-500 text-white shadow-lg shadow-violet-600/25 h-12 px-8">
      ...
      <footer className="border-t border-white/5 py-8">
        <div className="container flex flex-col items-center justify-between gap-4 px-4 text-xs text-zinc-500 sm:flex-row">
          <div className="flex items-center gap-2.5 font-display font-bold text-white">
            <div className="h-6 w-6 rounded border border-white/10 bg-white/5 grid place-items-center">
              <AudioWaveform className="h-3 w-3 text-violet-400" />
            </div>
            VOXAI
          </div>
