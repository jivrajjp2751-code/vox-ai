...
export default function Auth() {
  ...
  const subtitle = useMemo(
    () => (tab === "login" ? "Sign in to manage assistants and test voice live." : "Your assistants live in your private studio."),
    [tab],
  );
    ...
    try {
      if (tab === "login") {
        ...
        toast({
          ...
          description: "Confirm your email address to finish signup, then come back to sign in.",
        });
        setTab("login");
      }
    } catch (e: unknown) {
      ...
      <main className="container grid place-items-center py-10">
        <Card className="w-full max-w-md bg-background/70 shadow-pop backdrop-blur">
          ...
          <CardContent>
            <Tabs value={tab} onValueChange={(v) => setTab(v as any)}>
              ...
              <form className="mt-6 grid gap-4" onSubmit={form.handleSubmit(onSubmit)}>
                <TabsContent value="signup" className="m-0 grid gap-4">
          ...
          <CardFooter className="justify-between text-xs text-muted-foreground">
            ...
            <Link className="underline underline-offset-4 hover:text-foreground" to="/app">
              Skip →
            </Link>
          </CardFooter>
        </Card>
